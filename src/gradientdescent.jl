...
function FiniteDifferencesθ(ro, th, phi, DXsize, DYsize, pixels_x, pixels_y, SourceD, freq, maxnstep,h, bhspin, Rout, Rstop)
    """
    ...
    Imageh = IpoleGeoIntensityIntegration(trajectoryh, freq, pixels_x, pixels_y, scale_factor, bhspin)
    Imagel = IpoleGeoIntensityIntegration(trajectoryl, freq, pixels_x, pixels_y, scale_factor, bhspin)
...
function armijo_line_search!(cost_func, compute_gradients!, x, grad, direction, bounds, 
    ...
    for i in 1:max_steps
        ...
        println("Reducing step size by beta factor: $β")
        step_size *= β
        
        # If step becomes too small, return best point found
        if step_size < 1e-12
            println("  Step size too small, returning best point found")
